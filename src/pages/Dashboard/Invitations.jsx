...
export default function Invitations() {
  ...
  const InvCard = ({ inv, isRecue }) => {
    ...
    return (
      <div className="bg-blue-50 border border-blue-200 rounded-2xl p-5 flex flex-col gap-4 animate-slide-up">

        {/* Ligne principale */}
        <div className="flex items-start gap-4">
          {/* Icône */}
          <div className={`w-11 h-11 rounded-xl flex items-center justify-center text-xl flex-shrink-0
            ${isTuteurInvit ? 'bg-blue-700/20' : 'bg-blue-600/20'}`}>
            {isTuteurInvit ? '👨‍🏫' : '👤'}
          </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-slate-500 mt-0.5">
              {isRecue
                ? <>De <span className="text-blue-800 font-medium">{inv.expediteur_nom}</span></>
                : <>À <span className="text-blue-800 font-medium">{inv.destinataire_nom}</span></>
              }
              {' · '}
        ...
        {isTuteurInvit && (inv.matiere || inv.duree_heures || hasPaiement) && (
          <div className="bg-blue-100 rounded-xl p-4 flex flex-col gap-3">
  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        {/* Tabs */}
        <div className="flex gap-1 mb-6 bg-blue-50 border border-blue-200 rounded-xl p-1 w-fit">
          {tabs.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)}
              className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all flex items-center gap-2
                ${tab === t.id ? 'bg-blue-700 text-ink-800 shadow' : 'text-slate-500 hover:text-ink-600'}`}>
              {t.label}
              {t.count > 0 && (
                <span className={`text-xs px-1.5 py-0.5 rounded-full font-bold
                  ${tab === t.id ? 'bg-white/20' : 'bg-blue-700/30 text-blue-700'}`}>
                  {t.count}
                </span>
              )}
            </button>
